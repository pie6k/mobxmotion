...
`mobxmotion` is an opinionated and joyful library that helps you write semantic, composable, reusable styles using `React` and `styled-components`.
...
Install `mobxmotion` and use it to start styling your applications.
...
import { $flex, $animation } from "mobxmotion";
...
import { UI, $flex } from "mobxmotion";
import { animations } from "./styles";

function App() {
  return <UI.div styles={[animations.$fadeIn, $flex.horizontal.alignCenter.gap(2)]}>Hello World</UI.div>;
}
```
